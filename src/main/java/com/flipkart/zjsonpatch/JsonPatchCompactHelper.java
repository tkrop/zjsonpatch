...
final class JsonPatchCompactHelper {
    ...
    public void compact(final List<Diff> diffs) {
        for (int i = 0; i < diffs.size(); i++) {
            ...
            if (!(Operation.REMOVE.equals(diff1.getOp()) ||
                    Operation.ADD.equals(diff1.getOp()))) {
                continue;
            }
            ...
            for (int j = i + 1; j < diffs.size(); j++) {
                ...
                Diff moveDiff = null;
                if (Operation.REMOVE.equals(diff1.getOp()) &&
                        Operation.ADD.equals(diff2.getOp())) {
                ...
                } else if (Operation.ADD.equals(diff1.getOp()) &&
                        Operation.REMOVE.equals(diff2.getOp())) {
    ...
    private void computeRelativePath(List<Object> path, int startIdx, int endIdx, List<Diff> diffs) {
        ...
        for (int i = startIdx; i <= endIdx; i++) {
            ...
            if (Operation.ADD.equals(diff.getOp()) || Operation.REMOVE.equals(diff.getOp())) {
    ...
    private void updateCounters(Diff pseudo, int idx, List<Integer> counters) {
        if (Operation.ADD.equals(pseudo.getOp())) {
            counters.set(idx, counters.get(idx) - 1);
        } else {
            if (Operation.REMOVE.equals(pseudo.getOp())) {
                counters.set(idx, counters.get(idx) + 1);
            }
        }
    }
